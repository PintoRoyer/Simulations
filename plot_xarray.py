...
data = xr.open_mfdataset("/mesonh/panf/ADASTRA/CORSE/DX250/CORSE.1.SEG01.OUT.*1.nc")
...
for time in range (data.ACPRR.shape[0]):
    ...
    print("[" + n * "#" + (50 - n) * " " + "]", end="\r")
    contourf = axes.contourf(data.longitude[0], data.latitude[:, 0], data.ACPRR[time], cmap="Blues", levels=levels)
    date, hour = str(data.time[time].values).split("T")
    title = axes.text(0.5, 1.05, f"Simulation Méso-NH le {date} à {hour[: 5]} TU\n(dx = 250 m)", ha="center", transform=axes.transAxes)
