...
from readers import MesoNH, get_mesonh, get_time_index, index_to_lonlat
...
def plot_zoom_comparison_dx(lon: tuple, lat: tuple, hour: int, minute: int):
    """
    Plot a zoom from a lon/lat box at a given hour for the three horizontal resolutions on a unique
    figure and export it in a PNG file.

    Parameters
    ----------
    lon : tuple
        The longitude span (lon_min, lon_max).
    lat : tuple
        The latitude span (lat_min, lat_max).
    hour : int
        The hour to be plotted.
    minute : int
        The minute to be plotted .
    """
    plt.close("all")
    ...
    axes = plt.subplots(
        1, 3, figsize=(24, 5), layout="compressed", subplot_kw={"projection": ccrs.PlateCarree()}
    )[1]
    ...
    for index, resol_dx in enumerate((250, 500, 1000)):
        ...
        # Open Meso-NH files and extract requested hours
...
def run_all(resol_dx, args: Iterable, *, plot: bool = False, latex: bool = False):
    """
...
# print(lon, lat)
# plot_zoom_comparison_dx(lon, lat, hour, minute)
